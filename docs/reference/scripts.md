...
- `vp run install:desktop` — Builds the macOS app (`dir` target), quits any running copy, replaces `/Applications/T3 Code (Alpha).app`, and opens it. Flags: `--skip-build`, `--no-open`, `--verbose`.
