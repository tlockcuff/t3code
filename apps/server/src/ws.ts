          ...
          observeRpcStreamEffect(
            ...
            Effect.gen(function* () {
              ...
              if (input.afterSequence !== undefined) {
                ...
                // If the client resumes with an epoch that no longer matches the
                // server's (the DB was reset/restored, so sequences restarted
                // lower), a cursor-based replay would silently drop every future
                // event. Ignore the cursor and serve a fresh snapshot so the
                // client re-syncs from the new epoch. An absent epoch on either
                // side is a wildcard (no conflict), so old clients/servers keep
        ...
        [ORCHESTRATION_WS_METHODS.subscribeThread]: (input) =>
          observeRpcStreamEffect(
            ...
            Effect.gen(function* () {
              ...
              // Attach live delivery into a scope-bound buffer BEFORE reading any
              // snapshot or draining the catch-up replay, otherwise an event
              // published during that read window is lost (it is past the
              // persisted/DB state we just read, but the live subscription is not
              // attached yet). Every path below emits from this same buffered live
              // tail. Overlapping events are deduped by sequence on the client.
              ...
              // Offer the completion marker into the same queue as live events, so
              // anything buffered while snapshot/replay work was in flight is
              // delivered before the client is told it is synchronized.
              const synchronizedThenLive =
                input.requestCompletionMarker === true
                  ? Stream.concat(
                      Stream.fromEffect(
              ...
              if (input.afterSequence !== undefined) {
                ...
                // meaningless. Serve a fresh snapshot instead of replaying from a
                // sequence that no longer refers to the same log. An absent epoch
                // on either side is a wildcard (no conflict).
                const { epoch: serverEpoch } = yield* projectionSnapshotQuery
                  .getSnapshotSequence()
                  .pipe(
                    Effect.mapError(
                ...
                if (
                  ...
                  return Stream.concat(Stream.make(snapshotFrame), synchronizedThenLive);
                }
