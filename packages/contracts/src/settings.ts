...
export const ServerSettings = Schema.Struct({
  ...
  // Active Space label, shared by every client of this server so switching
  // spaces on one device follows to the others. `null` = "All projects";
  // stale labels (no current project carries them) behave as "All".
  activeSpace: Schema.NullOr(Schema.String).pipe(Schema.withDecodingDefault(Effect.succeed(null))),
});
...
export const ServerSettingsPatch = Schema.Struct({
  ...
  activeSpace: Schema.optionalKey(Schema.NullOr(Schema.String)),
});
